/**
 * Enriches parsed itinerary with real place data using Gemini with Google Search grounding
 * Note: googleMaps is NOT a valid Gemini tool. Only googleSearch grounding exists.
 ...
 * Takes a parsed itinerary and enriches items with details
 * Supports incremental enrichment when existingEnrichment is provided
 ...
 * @param {Object|null} existingEnrichment - Previously enriched itinerary for incremental updates
 * @returns {Promise<Object>} Enriched itinerary with place details
 */
async function enrichItinerary(parsed, genAI, existingEnrichment = null) {
  ...
  try {
    ...
    if (existingEnrichment) {
      // INCREMENTAL: Only enrich items that are new or changed
      const existingMap = extractExistingEnrichmentMap(existingEnrichment);
      const itemsToEnrich = filterItemsNeedingEnrichment(items, existingMap);

      if (itemsToEnrich.length === 0) {
        console.log('All items already enriched, skipping Gemini call');
        enrichedItems = existingMap;
      } else {
        console.log(`Enriching ${itemsToEnrich.length} of ${items.length} items (incremental)`);
        const newEnrichments = await batchEnrichItems(itemsToEnrich, genAI);
        // Merge new enrichments with existing
        enrichedItems = { ...existingMap, ...newEnrichments };
      }
    } else {
      // FULL: Enrich all items (startup or full replace)
      console.log(`Full enrichment: ${items.length} items`);
      enrichedItems = await batchEnrichItems(items, genAI);
    }
    ...
    // On error, try to preserve existing enrichment if available
    if (existingEnrichment) {
      console.log('Falling back to existing enrichment data');
      const existingMap = extractExistingEnrichmentMap(existingEnrichment);
      return mergeEnrichedData(parsed, existingMap);
    }
    return convertToDisplayFormat(parsed);
  }
}
...
async function batchEnrichItems(items, genAI) {
  ...
  try {
    ...
    // Note: googleMaps is NOT a valid Gemini tool - only googleSearch exists
...
/**
 * Extract a description -> place map from existing enriched data
 * Used for incremental enrichment to preserve existing data
 * @param {Object} enriched - Previously enriched itinerary
 * @returns {Object} Map of description -> place data
 */
function extractExistingEnrichmentMap(enriched) {
  if (!enriched) return {};
  const map = {};

  // Extract hotel enrichment
  if (enriched.hotel && typeof enriched.hotel === 'object' && enriched.hotel.name) {
    map[enriched.hotel.name] = enriched.hotel;
  }

  // Extract daily item enrichments
  for (const day of (enriched.days || [])) {
    for (const item of (day.items || [])) {
      if (item.place && item.description) {
        map[item.description] = item.place;
      }
    }
  }

  // Extract reservation enrichments
  for (const res of (enriched.reservations || [])) {
...
/**
 * Filter items to only those needing enrichment
 * Items are skipped if they have complete existing enrichment
 * @param {Array} items - All items from extractAllItems()
 * @param {Object} existingMap - Map from extractExistingEnrichmentMap()
 * @returns {Array} Items that need enrichment
 */
function filterItemsNeedingEnrichment(items, existingMap) {
  return items.filter(item => {
    const existing = existingMap[item.description];
    // Needs enrichment if no existing data
    if (!existing) return true;
    // Needs enrichment if marked as needing details
    if (existing.needsDetails) return true;
    // Needs enrichment if has no meaningful content
    if (!existing.address && !existing.description && !existing.waypoints?.length) return true;
    return false;
  });
}
...
module.exports = {
  ...
  extractExistingEnrichmentMap,
  filterItemsNeedingEnrichment
};
