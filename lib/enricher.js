...
function looksLikePlace(description) {
  ...
  const excluded = [
    'check-in', 'sleep', 'walk', 'rest', 'break', 'arrive', 'depart',
    'pack', 'prepare', 'relax', 'explore', 'wander', 'browse'
  ];
...
async function batchEnrichPlaces(places, genAI) {
  ...
  try {
    ...
    const prompt = `You are a NYC travel expert. For each place listed below, provide the following information in JSON format:
- name: the official name of the place
- address: the street address in NYC
- mapsUrl: a Google Maps URL (format: https://maps.google.com/?q=PLACE+NYC)
- website: the official website URL (if known, or empty string)
- tips: 1-2 practical tips for visiting (e.g., hours, booking, what to do there)

Return only valid JSON as an array of objects, one per place. Do not include markdown formatting.
