...
const tools = [
  ...
  },
  {
    ...
    description: 'Get hotel/accommodation information including name, address, and neighborhood. Use this to find places near the hotel or answer questions about where the travelers are staying.',
    parameters: {
      type: Type.OBJECT,
      properties: {},
      required: []
  ...
  {
    name: 'getExpenses',
    description: 'Get the trip expenses and totals. Shows all expenses with who paid (George or Val), amounts, and calculates who owes whom.',
    parameters: {
      type: Type.OBJECT,
      properties: {},
      required: []
...
function createOscarAgent(dependencies) {
  ...
  /**
   * Format accommodation data for inclusion in system prompt
   * @param {Array} accommodations - Array of accommodation objects
   * @returns {string} Formatted accommodation text
   */
  function formatAccommodationForContext(accommodations) {
    if (!accommodations || accommodations.length === 0) {
      return 'No accommodation information available.';
    }

    let output = '';
    for (const acc of accommodations) {
      output += `- **${acc.name}**`;
      if (acc.neighborhood) output += ` (${acc.neighborhood})`;
      output += '\n';
      if (acc.address) output += `  Address: ${acc.address}\n`;
      output += `  Dates: ${acc.checkIn} to ${acc.checkOut}\n`;
    }
    return output;
  }
  ...
  async function buildSystemPrompt(userId, itineraryDays = null) {
    ...
    if (userId) {
      try {
        accommodations = await db.getAccommodationsByUser(userId);
      } catch (err) {
        console.warn('Could not load accommodations for context:', err.message);
      }
    }

    const itineraryContext = formatItineraryForContext(days);
    const accommodationContext = formatAccommodationForContext(accommodations);
...
- getAccommodation: Get hotel details (you already have this below, but can refresh if needed)

EXPENSE TRACKING TOOLS:
- getExpenses: See all trip expenses, totals, and who owes whom
- addExpense: Log a new expense (description, amount, who paid)
...
4. You already know the ACCOMMODATION details below - use this when users ask about things "near hotel" or "near where we're staying"
5. When user wants to change plans, use updateItinerary directly (no need to call getItinerary first - you have the IDs below)
6. Be specific: include addresses, hours, price range from your searches
7. When adding/updating items, you need the item ID - it's shown in [id: xxx] format in the itinerary below
8. For expenses: when users mention paying for things, offer to log it
9. Payers are George and Val - ask who paid if not clear
...
ACCOMMODATION (where they're staying):
...
${itineraryContext}`;
  ...
  /**
   * Execute getAccommodation tool - get hotel/accommodation info
   * @param {Object} args - Tool arguments (unused)
   * @param {string} userId - User ID from context
   */
  async function executeGetAccommodation(args, userId) {
    if (!userId) {
      return {
        success: false,
        error: 'User ID required to fetch accommodation'
      };
    }
    ...
    try {
      ...
      if (!accommodations || accommodations.length === 0) {
        return {
          success: true,
          accommodations: [],
          message: 'No accommodation information on file.'
        };
      }
      ...
      console.error('getAccommodation error:', err.message);
      return {
        success: false,
        error: err.message
      };
   ...
   * @param {Object} args - Tool arguments with description, amount, payer, date
   * @param {string} userId - User ID from context
   */
  async function executeAddExpense(args, userId) {
    const { description, amount, payer, date } = args;
  ...
  async function executeUpdateExpense(args, userId) {
    const { expenseId, description, amount, payer, date } = args;
  ...
  async function executeTool(name, args, context) {
    ...
    switch (name) {
      ...
      case 'getAccommodation':
        return await executeGetAccommodation(args, userId);
      case 'getExpenses':
