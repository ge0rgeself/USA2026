 ...
 *
 * Uses Gemini 3 Flash for reasoning/chat with function calling
...
${preferences}`;
  ...
  /**
   * Execute searchPlaces tool using Gemini 2.5 Flash with Maps grounding
   */
  async function executeSearchPlaces(args) {
    const { query, neighborhood } = args;

    try {
      // Use Gemini 2.5 Flash with Maps grounding
      const response = await genAI.models.generateContent({
        model: mapsModel,
        contents: `Find ${query}${neighborhood ? ` in/near ${neighborhood}` : ''} in New York City.
                   Return the top 3-5 best options with: name, address, neighborhood, hours, price range ($ to $$$$), rating, and a brief description of what makes it special.
                   Focus on places that would be open in January 2025.`,
        config: {
          tools: [{ googleMaps: {} }],
          toolConfig: {
            retrievalConfig: {
              latLng: { latitude: 40.7459, longitude: -73.9880 } // NoMad/Midtown default
            }
          }
        }
      });
   ...
   */
  function executeGetItinerary(args, itineraryJson) {
    const { day } = args;
    ...
    if (day) {
      ...
      if (dayIndex !== undefined && itineraryJson.days[dayIndex]) {
        const dayData = itineraryJson.days[dayIndex];
        return {
          success: true,
          day: dayData.date,
          title: dayData.title,
    ...
    return {
      success: true,
      hotel: itineraryJson.hotel,
      days: itineraryJson.days.map(d => ({
        date: d.date,
        title: d.title,
  ...
  async function executeTool(name, args, context) {
    ...
    switch (name) {
      ...
      case 'getItinerary':
        return executeGetItinerary(args, context.itineraryJson);
      default:
  ...
  async function chat(message, context) {
    const { chatHistory = [], itineraryJson } = context;
        ...
        if (response.functionCalls && response.functionCalls.length > 0) {
          ...
          for (const functionCall of response.functionCalls) {
            const result = await executeTool(
              functionCall.name,
              functionCall.args,
              { itineraryJson, itineraryManager }
            );
            toolResults.push({
