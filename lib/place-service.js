...
const CONFIG = {
  ...
  hotelLocation: { latitude: 40.7223, longitude: -73.9930 }, // Freeman Alley, LES
  maxBatchSize: 10,
  maxRetries: 3
};
...
function buildEnrichmentPrompt(items) {
  ...
  return `You are enriching places for a NYC trip (Jan 14-18, 2026).
...
FOR ACTIVITIES (non-place items like "walk around LES", "Sleep in", "Check-in"), return:
{
  "name": "Clean display title (e.g., 'Lower East Side Walk', 'Sleep In', 'Hotel Check-in')",
  "hook": "Brief contextual note or activity tip",
...
Focus on places open in January 2026.`;
